...
class ConvolutionBFM(metaclass=utility_classes.Singleton):
    ...
    async def reset(self):
        ...
        self.dut.kernel_weights.value = 0
        self.dut.kernel_size.value = 3
        self.dut.stride.value = 1
        self.dut.padding.value = 0
        self.dut.img_height.value = 28
        self.dut.img_width.value = 28
        self.dut.in_channels.value = 1
        self.dut.out_channels.value = 1
        self.dut.activation.value = 0
        await RisingEdge(self.dut.clk)
    ...
    async def driver_bfm(self):
        ...
        while True:
            ...
            try:
                ...
                self.dut.kernel_weights.value = packed_kernel
                
                # Set the configurable parameters from config
                self.dut.kernel_size.value = config['kernel_size']
                self.dut.stride.value = config.get('stride', 1)
                self.dut.padding.value = config.get('padding', 0)
                self.dut.img_height.value = config['input_shape'][0]
                self.dut.img_width.value = config['input_shape'][1]
                self.dut.in_channels.value = config.get('input_channels', 1)
                self.dut.out_channels.value = config.get('output_channels', 1)
                
                # Set activation function
                activation_map = {'none': 0, 'relu': 1, 'sigmoid': 2}
                activation = config.get('activation', 'none')
                self.dut.activation.value = activation_map.get(activation, 0)
